    ...
    if uploaded_file:
        ...
        with st.spinner("Analyzing..."):
            ...
            if predicted_class == "healthy":
                ...
                st.write(f":red[Early stage of MENINGIOMA.]")
                col1, col2, col3= st.columns(3)
                with col2:
                    st.write("Prediction confidence: ", round(np.max(prediction) * 100, 2), " %")
                col1, col2= st.columns(2)
                with col1:
                    st.write("Key Insights: ")
                    st.write("- Enhancing mass on the outside lining of the brain")
                    st.write("- Homogeneous and well-circumscribed")
                with col2:
                    st.write("Preventive measures: ")
                    st.write("- Please consult a doctor.")
                    st.write("- SRS(Stereotactic Radiosurgery)")
